clc;
clear all;
nodes_per_cluster = 5; % Number of nodes in each cluster
total_clusters = 6;    % Total number of clusters
total_nodes = nodes_per_cluster * total_clusters; % Total number of nodes
...
x_loc = zeros(1, total_nodes);
y_loc = zeros(1, total_nodes);
...
for cluster = 1:total_clusters
    ...
    idx_start = (cluster - 1) * nodes_per_cluster + 1;
    idx_end = cluster * nodes_per_cluster;
    
    % Position nodes randomly within each cluster's area
    x_loc(idx_start:idx_end) = col * cluster_length + cluster_length * rand(1, nodes_per_cluster);
    y_loc(idx_start:idx_end) = row * cluster_width + cluster_width * rand(1, nodes_per_cluster);
    
    % Calculate remaining energy for each node in the cluster
    distances = sqrt((x_loc(idx_start:idx_end) - mean(x_loc(idx_start:idx_end))).^2 + (y_loc(idx_start:idx_end) - mean(y_loc(idx_start:idx_end))).^2);
    node_energy = initial_energy - (E_Tx_circuit * b + E_Tx_Amp * b .* distances.^n + E_DA);
    
    % Select the cluster head based on the highest energy
    [~, idx] = max(node_energy);
    cluster_heads(cluster) = idx_start + idx - 1;
end
...
title('Six Cluster Network with Star Topology');
grid on;
